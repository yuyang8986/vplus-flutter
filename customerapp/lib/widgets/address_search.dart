    ...
    return Container(
      child: TextField(
        ...
        decoration: CustomTextBox(
          ...
          mandate: true,
          hint:
              "${AppLocalizationHelper.of(context).translate("addressEntry")}",
        ...
        onTap: () async {
          ...
          if (result == null) {
            try {
              ...
              setState(() {
                ...
                _streetNumber = placeDetails.split(",")[0].split(" ")[0] ?? "";
                _street = placeDetails.split(",")[0].split(" ")[1] +
                        " " +
                        placeDetails.split(",")[0].split(" ")[2] ??
                    "";
                _city = placeDetails.split(",")[1] ?? "";
                _zipCode = placeDetails.split(",")[2].split(" ")[1] ??
              ...
              widget.onSetAddress(addressController.text, prevLocation);
            } catch (e) {
              print(e.toString());
            }
          }
          ...
          // result with auto suggest
            ...
            final placeDetails = await PlaceApiHelper(sessionToken)
                .getPlaceDetailFromId(result.placeId);
            Coordinates coord =
                new Coordinates(placeDetails.lat, placeDetails.lng);
            setState(() {
              //addressController.text = result.description;
              // street number can be null, allow store address correct to street
            ...
            widget.onSetAddress(addressController.text, coord);
          }
        ...
        builder: (context, snapshot) {
          if (query == '') {
            return Container(
              ...
              child: Text(
                  "${AppLocalizationHelper.of(context).translate("Enter your address")}"),
            );
          }
          if (snapshot.hasData) {
            _data = snapshot.data;
          ...
          return Container(
              child: Text(
                  "${AppLocalizationHelper.of(context).translate("Loading...")}"));
        });
  }
}
